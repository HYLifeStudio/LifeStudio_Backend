...
public class FileService {
    ...
    public Photo upload(MultipartFile multipartFile, String dirName) throws IOException {
    ...
    // S3로 파일 업로드하기
    private Photo upload(File uploadFile, String dirName) {
        String fileName = dirName + "/" + UUID.randomUUID() + uploadFile.getName();   // S3에 저장된 파일 이름
        String uploadImageUrl = putS3(uploadFile, fileName); // s3로 업로드
        removeNewFile(uploadFile);
        Photo photo = Photo.builder()
                .title(uploadFile.getName())
                .url(uploadImageUrl)
                .build();
        return photo;
    }
